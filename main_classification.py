...
def get_args_parser():
    ...
    parser.add_option("--num_class", dest="num_class", help="number of the classes in the downstream task",
                      default=14, type="int")
    parser.add_option("--data_set", dest="data_set", help="ChestXray14|CheXpert|Shenzhen|VinDrCXR|RSNAPneumonia|RSNAPe", default="ChestXray14", type="string")
...
def main(args):
    ...
    elif args.data_set == "RSNAPe":
        diseases = ['PE']
        dataset_train = PEDataset(data_dir=args.data_dir, file_path=args.train_list,
                                  target_size=args.img_size, mode="train")

        dataset_val = PEDataset(data_dir=args.data_dir, file_path=args.val_list,
                                target_size=args.img_size, mode="val")

        dataset_test = PEDataset(data_dir=args.data_dir, file_path=args.test_list,
                                 target_size=args.img_size, mode="test")
